class Scene6 {
  ...
  float ghostAlpha = 255; // Start with full transparency
  ...
  void display() {
    ...
    // Position above the cliff (150 is the height of the cliff)
    drawPocong(290, 255, 1);
  ...
  void drawPocong(float x, float y, float scale) {
    pushMatrix();
    translate(x, y);
    scale(scale);
  ...
  void drawGhost(float x, float y, float scale, float alpha) {
    ...
    // Debug: Add a circle to ensure position is correct
    fill(255, 0, 0, alpha); // Red color with alpha
    ellipse(0, 0, 10, 10);
    ...
    // Add two legs to ghost with kebaya
    fill(255, 228, 196, alpha); // Pale skin color
    rect(-20, 50, 10, 30); // Left leg
    rect(10, 50, 10, 30); // Right leg

    // Hands with vampire shape
    fill(144, 238, 144, alpha); // Light green color with transparency
    beginShape();
    vertex(-50, -50); // Left upper hand
    vertex(-70, -30);
    vertex(-40, -10);
    vertex(-30, -20);
    vertex(-50, -50);
